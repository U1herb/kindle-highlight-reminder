...
Notion の `05_book` データベースを入力ソースにして、3冊の本と各3つの読書メモを毎日ランダムに表示・通知するPWAです。
...
ライブラリは `05_book` の全ページを対象にします。ハイライトがまだ抽出されていない本もライブラリに表示し、今日の3冊の抽選対象はハイライトが1件以上ある本だけにします。
...
- Current full library size confirmed through MCP: 315 books
